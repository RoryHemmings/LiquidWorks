...
class Editor extends Scene {
    ...
    my_mouse_down_rotate(e, pos, context, program_state, initial) {
        let pos_ndc_near = vec4(pos[0], pos[1], 0, 1.0);
        let pos_ndc_far  = vec4(pos[0], pos[1], 0, 1.0);
        let center_ndc_near = vec4(0.0, 0.0, 1.0, 1.0);
        let P = program_state.projection_transform;
        let V = program_state.camera_inverse;
        let pos_world_near = Mat4.inverse(P.times(V)).times(pos_ndc_near);
    ...
    display(context, program_state) {
        ...
        if (!context.scratchpad.controls) {
            ...
            let LookAt = Mat4.look_at(vec3(2, 3, 10), vec3(0, 0, 0), vec3(0, 1, 0));
        ...
        this.shapes.cube.draw(context, program_state, Mat4.scale(100, .05, .05), this.materials.phong);
        this.shapes.cube.draw(context, program_state, Mat4.scale(.05, 100, .05), this.materials.phong);
        this.shapes.cube.draw(context, program_state, Mat4.scale(.05, .05, 100), this.materials.phong);
